...
NumericMatrix raster_data(int iter, int layers, int pixels, double zoom, double alpha) {
  ...
  double y_offset = R::runif(-1, 1);
  double x_offset = R::runif(-1, 1);
  int n_sym = 3 + (rand() % 10);
  int rr = 0;
  NumericVector r_offset(n_sym); 
  for(rr = 0; rr < n_sym; rr++) {
    //r_offset(rr) = R::runif(0, 6.28); // randomly spaced
    //r_offset(rr) = (rr / n_sym) * 6.28; // evenly spaced
    if(rand() % 3 == 0) {
      r_offset(rr) = R::runif(0, 6.28); // randomly spaced
    } else {
      r_offset(rr) = (rr / n_sym) * 6.28; // evenly spaced  
  ...
  for(int it = 1; it < iter; it++) {
    ...
    // variants 1-3 are the celestial spheres 
    } else if(variant == 1){
      ...
      x = x_tmp * cos(r_offset(0)) - y_tmp * sin(r_offset(0));
      y = x_tmp * sin(r_offset(0)) + y_tmp * cos(r_offset(0));
      ...
      // convert to x,y coords of a circle
      rr = rand() % n_sym;
      x = x_tmp * cos(r_offset(rr)) - y_tmp * sin(r_offset(rr));
      y = x_tmp * sin(r_offset(rr)) + y_tmp * cos(r_offset(rr));
    ...
    // variants 4-5 are the lissajous attractor 
    } else if(variant == 4){
      s = x * y;
      x_tmp = cos(s);
      if((rand() % 2) == 0) {
